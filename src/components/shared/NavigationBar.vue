<template>
  <nav
    class="fixed inset-x-0 top-0 z-[100] border-b border-border-color bg-bg-primary/95 backdrop-blur-2xl transition-all duration-200 ease-standard">
        ...
        <div v-if="isAuthenticated" class="hidden lg:block relative" ref="userMenuRef">
          ...
          <Transition enter-active-class="transition-all duration-200 ease-out"
            ...
            <div v-if="userMenuOpen"
              ...
              <router-link to="/history" @click="userMenuOpen = false"
                class="flex items-center gap-3 px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <Transition enter-active-class="transition-all duration-300 ease-out"
      ...
      <div v-if="mobileMenuOpen"
        class="lg:hidden fixed left-0 right-0 top-[70px] bottom-0 z-[95] bg-bg-primary backdrop-blur-2xl">
        <div class="container py-6 bg-gradient-to-b from-bg-primary via-bg-primary to-bg-elevated">
          <!-- Search Bar (Mobile) -->
          <div class="mb-6">
            <div class="relative">
              <input type="text" placeholder="Cari manhwa favorit Anda..."
    ...
    <!-- Auth Modal -->
    <AuthModal :is-open="showAuthModal" @close="showAuthModal = false" @success="showAuthModal = false" />
  </nav>
</template>
