<template>
  <div class="min-h-screen bg-[var(--bg-primary)] pt-[70px]">
    <!-- Page Header -->
    <section
      class="border-b border-[var(--border-color)] bg-gradient-to-br from-[rgba(139,92,246,0.1)] to-[rgba(167,139,250,0.05)] py-10 sm:py-12">
      <div class="container">
        <div class="mb-8 flex flex-col items-start gap-6 sm:flex-row sm:items-center">
          <div
            class="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-[var(--accent-primary)] to-[var(--accent-secondary)] text-white [box-shadow:var(--shadow-lg),var(--glow-accent)] sm:h-20 sm:w-20">
          ...
          <div>
            ...
            <p class="text-base text-[var(--text-secondary)] sm:text-lg">Manhwa paling populer dan paling banyak dibaca
            </p>
          </div>
        </div>
        ...
        <div class="flex gap-2 overflow-x-auto pb-2 sm:gap-3">
          <button v-for="option in filterOptions" :key="option.value" :class="[
    ...
    <section class="py-12">
      <div class="container">
        ...
        <div v-if="loading" class="flex flex-col items-center justify-center gap-4 px-8 py-16 text-center">
          <div
            class="h-12 w-12 animate-spin rounded-full border-4 border-[var(--border-color)] border-t-[var(--accent-primary)]">
          </div>
          <p class="text-base text-[var(--text-secondary)]">Memuat manhwa populer...</p>
        </div>

        <div v-else-if="displayedManhwa.length === 0"
          class="flex flex-col items-center justify-center px-8 py-16 text-center">
          <svg class="mb-6 h-20 w-20 text-[var(--text-muted)] opacity-50" viewBox="0 0 24 24" fill="none"
        ...
        <div v-else>
          <div
            class="mb-12 grid gap-4 [grid-template-columns:repeat(auto-fill,minmax(150px,1fr))] sm:gap-6 sm:[grid-template-columns:repeat(auto-fill,minmax(200px,1fr))]">
            <ManhwaCard v-for="(manhwa, index) in displayedManhwa" :key="manhwa.slug" :slug="manhwa.slug"
              ...
              :genre="manhwa.genre" :badge="manhwa.badge || (index < 3 ? 'Hot' : 'Popular')"
              :lastUpdate="manhwa.lastUpdate" :latestChapters="manhwa.latestChapters" @click="goToDetail"
