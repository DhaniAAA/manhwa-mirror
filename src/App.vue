...
<template>
  <div class="flex min-h-screen flex-col bg-[var(--bg-primary)]">
    ...
    <main v-if="!showReader" class="relative z-[1] flex-1">
      ...
      <ManhwaSection title="Lanjutkan Membaca" subtitle="Lanjutkan dari terakhir kali Anda membaca" :showViewAll="true"
        :manhwaList="continueManhwa" @cardClick="openDetail" @quickRead="openDetail" />

      <ManhwaSection title="Trending Sekarang" subtitle="Manhwa paling populer minggu ini" :showViewAll="true"
        :manhwaList="trendingManhwa" @cardClick="openDetail" @quickRead="openDetail" />

      <ManhwaSection title="Rilis Terbaru" subtitle="Chapter terbaru yang baru saja dirilis" :showViewAll="true"
        :manhwaList="newReleases" @cardClick="openDetail" @quickRead="openDetail" />
    </main>
    ...
    <footer v-if="!showReader"
      ...
      <div class="container">
        <div class="mb-12 grid gap-12 lg:[grid-template-columns:2fr_3fr] lg:gap-16">
          <div class="max-w-md space-y-4">
            <div class="flex items-center gap-3 text-xl font-bold text-[var(--text-primary)]">
              <img src="/Logo.png" alt="Manhwaku" width="50" height="50" class="rounded-lg" />
              <span class="bg-gradient-to-r from-[#8b5cf6] to-[#a78bfa] bg-clip-text text-transparent">Manhwaku</span>
            </div>
          ...
          <div class="grid gap-8 md:grid-cols-3">
            <div class="space-y-3">
              ...
              <a class="block text-sm text-[var(--text-secondary)] transition-colors duration-200 hover:text-[var(--accent-primary)] hover:underline"
                href="#">Beranda</a>
              <a class="block text-sm text-[var(--text-secondary)] transition-colors duration-200 hover:text-[var(--accent-primary)] hover:underline"
                href="#">Perpustakaan</a>
              <a class="block text-sm text-[var(--text-secondary)] transition-colors duration-200 hover:text-[var(--accent-primary)] hover:underline"
        ...
        <div
          class="flex flex-col gap-6 border-t border-[var(--divider-color)] pt-8 text-center md:flex-row md:items-center md:justify-between md:text-left">
          <p class="text-sm text-[var(--text-muted)]">&copy; 2025 Manhwaku. Semua hak dilindungi.</p>
    ...
    <!-- Detail Page -->
    <ManhwaDetail v-if="showDetail && selectedManhwa" :slug="selectedManhwa.slug || ''"
      :title="metadata?.title || selectedManhwa.title" :rating="metadata?.rating || selectedManhwa.rating"
      :totalChapters="metadata?.total_chapters || selectedManhwa.chapters || selectedManhwa.total_chapters || 0"
      :description="metadata?.description" :author="metadata?.author" :artist="metadata?.artist"
      :genres="metadata?.genres || selectedManhwa.genre?.split(', ') || []" :status="metadata?.status"
      :type="metadata?.type" :releaseYear="metadata?.release_year"
      ...
      @close="closeDetail" @readChapter="openReaderFromDetail" />

    <!-- Reader -->
    <ManhwaReader v-if="showReader" :manhwaTitle="selectedManhwa?.title || ''" :manhwaSlug="selectedManhwa?.slug || ''"
      @close="closeReader" />
  </div>
</template>
